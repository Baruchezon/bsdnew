...
function normalizeBrand(){document.querySelectorAll('.nav .brand').forEach(brand=>{if(!brand.querySelector('img')){const img=document.createElement('img');img.src=LOGO;img.alt='לוגו מרכז עדן';brand.prepend(img)}if(!brand.querySelector('strong')){const s=document.createElement('strong');s.textContent='מרכז עדן';brand.appendChild(s)}});document.querySelectorAll('.menu a[href*="eden-baby"]').forEach(a=>a.textContent='שחיית תינוקות');document.querySelectorAll('.menu a').forEach(a=>{const here=location.pathname.split('/').pop()||'index.html';const there=a.getAttribute('href').split('/').pop();if(here===there)a.classList.add('active')})}
...
if(form){form.addEventListener('submit',async e=>{
  ...
  try{
    ...
    void sendLeadEmailNotification().catch(()=>{});
  }catch(err){
  ...
  }finally{btn.disabled=false;btn.textContent='שליחת פנייה למרכז עדן';}
});}
// cooking-baking-nav
