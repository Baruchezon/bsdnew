/* Privacy controls. Basic anonymous traffic counting runs without optional cookies. */
(() => {
  'use strict';
  ...
  try {
    ...
    if (value && value.version === version && typeof value.analytics === 'boolean' &&
        Number.isFinite(value.savedAt) && value.savedAt <= Date.now() && Date.now() - value.savedAt < lifetime) {
      choice = value;
    } else {
      localStorage.removeItem(key);
    }
  } catch (_) { /* Storage may be blocked; optional analytics stays disabled. */ }
  ...
  function setup() {
    ...
    // Always load the basic anonymous counter. Optional dimensions remain consent based.
    loadBasicAnalytics();
  }
